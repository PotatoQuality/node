...
const {
  Math,
  Object,
  StringPrototype: { endsWith, repeat, slice: stringSlice, split },
} = primordials;
  ...
  for (i = 0; i < maxLines; i++) {
    ...
    // Lines diverge
      ...
      let divergingLines = actualLine !== expectedLine &&
                           (!endsWith(actualLine, ',') ||
                            stringSlice(actualLine, 0, -1) !== expectedLine);
      ...
      if (divergingLines &&
          ...
          stringSlice(expectedLine, 0, -1) === actualLine) {
  ...
  constructor(options) {
    ...
    if (message != null) {
      ...
      if (operator === 'deepStrictEqual' || operator === 'strictEqual') {
        ...
        const res = split(inspectValue(actual), '\n');
  ...
  [inspect.custom](recurseTimes, ctx) {
    ...
    for (const name of ['actual', 'expected']) {
      if (typeof this[name] === 'string') {
        const lines = split(this[name], '\n');
        if (lines.length > 10) {
          lines.length = 10;
          this[name] = `${lines.join('\n')}\n...`;
        } else if (this[name].length > 512) {
          this[name] = `${stringSlice(this[name], 512)}...`;
        }
      }
    }
