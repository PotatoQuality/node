...
(function bootstrapInternalLoaders(process, getBinding, getLinkedBinding,
  ...
  function NativeModule(id) {
    ...
    this.loaded = false;
    this.loading = false;
    this.compiledFunction = null;  // The compiled function of the module
  }
  ...
  NativeModule.prototype.compile = function() {
    ...
    try {
      ...
      //    `internalBinding('natives_hash')`, then the code used to generate
      //    `internalBinding('code_cache')` should be in sync with the code in
      //    `process.binding('natives')`
      const cache = codeCacheHash[id] &&
        (codeCacheHash[id] === sourceHash[id]) ? codeCache[id] : Buffer.alloc(100000);

      // (code, filename, lineOffset, columnOffset, cachedData,
      // produceCachedData, parsingContext, contextExtensions, params)
      const compiledFunction = compileFunction(
        source, this.filename, 0, 0, cache,
        true, undefined, [], ['exports', 'require', 'module', 'process']
      );
      ...
      if (!cache || compiledFunction.cachedDataRejected) {
      ...
      compiledFunction(this.exports, requireFn, this, process);
