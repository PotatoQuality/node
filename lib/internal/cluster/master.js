...
const {
  Object,
  StringPrototype: { match, startsWith },
} = primordials;
...
cluster.setupMaster = function(options) {
  ...
  if (settings.execArgv.some((s) => startsWith(s, '--prof')) &&
      !settings.execArgv.some((s) => startsWith(s, '--logfile='))) {
...
function createWorkerProcess(id, env) {
  ...
  if (execArgv.some((arg) => match(arg, debugArgRegex)) ||
      match(nodeOptions, debugArgRegex)) {
    let inspectPort;
