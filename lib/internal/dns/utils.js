...
class Resolver {
  ...
  setServers(servers) {
    ...
    servers.forEach((serv, index) => {
      ...
      const ipV6Match = match(serv, IPv6RE);

      // Check for an IPv6 in brackets.
      if (ipV6Match) {
        ipVersion = isIP(ipV6Match[1]);

        if (ipVersion !== 0) {
          const port =
            parseInt(replace(serv, addrSplitRE, '$2')) || IANA_DNS_PORT;
          return newSet.push([ipVersion, ipV6Match[1], port]);
        }
      }

      // addr::port
      const addrSplitMatch = match(serv, addrSplitRE);
