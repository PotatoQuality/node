...
const {
  ...
  Object,
  SafeMap,
  StringPrototype: { replace, slice: stringSlice },
} = primordials;
...
translators.set('commonjs', async function commonjsStrategy(url, isMain) {
  ...
  const module = CJSModule._cache[
    isWindows ? replace(pathname, winSepRegEx, '\\') : pathname
  ];
...
translators.set('json', async function jsonStrategy(url) {
  ...
  const modulePath = isWindows ?
    replace(pathname, winSepRegEx, '\\') : pathname;
