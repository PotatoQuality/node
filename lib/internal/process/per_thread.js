...
const {
  ...
  RegExpPrototype,
  SetPrototype,
  StringPrototype: { endsWith, replace, startsWith, substr },
} = primordials;
...
function buildAllowedFlags() {
  ...
  for (const [ from, expansion ] of aliases) {
    ...
    if (isAccepted) {
      let canonical = from;
      if (endsWith(canonical, '='))
        canonical = substr(canonical, 0, canonical.length - 1);
      if (endsWith(canonical, ' <arg>'))
        canonical = substr(canonical, 0, canonical.length - 4);
