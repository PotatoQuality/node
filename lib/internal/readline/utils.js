  ...
  while (true) {
    ...
    if (escaped && (ch === 'O' || ch === '[')) {
      ...
      if (ch === 'O') {
         ...
         */
        const cmd = stringSlice(s, cmdStart);
        let cmdMatch;

        if ((cmdMatch = match(cmd, /^(\d\d?)(;(\d))?([~^$])$/))) {
          code += cmdMatch[1] + cmdMatch[4];
          modifier = (cmdMatch[3] || 1) - 1;
        } else if ((cmdMatch = match(cmd, /^((\d;)?(\d))?([A-Za-z])$/))) {
          code += cmdMatch[4];
          modifier = (cmdMatch[3] || 1) - 1;
        } else {
          code += cmd;
      ...
      // ctrl+letter
      key.name = fromCharCode(charCodeAt(ch, 0) + charCodeAt('a', 0) - 1);
      key.ctrl = true;
    } else if (/^[0-9A-Za-z]$/.test(ch)) {
      // Letter, number, shift+letter
      key.name = toLowerCase(ch);
