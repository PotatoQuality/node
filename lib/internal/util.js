...
const {
  Object,
  Reflect,
  StringPrototype: { replace, toLowerCase, toUpperCase },
} = primordials;
...
function removeColors(str) {
  return replace(str, colorRegExp, '');
}
...
function slowCases(enc) {
  switch (enc.length) {
    case 4:
      ...
      break;
    case 3:
      if (enc === 'hex' || enc === 'HEX' || toLowerCase(`${enc}`) === 'hex')
        return 'hex';
      break;
    case 5:
    ...
    case 7:
      if (enc === 'utf16le' || enc === 'UTF16LE' ||
        toLowerCase(`${enc}`) === 'utf16le')
        return 'utf16le';
      break;
    case 8:
      if (enc === 'utf-16le' || enc === 'UTF-16LE' ||
        toLowerCase(`${enc}`) === 'utf-16le')
        return 'utf16le';
      break;
    default:
      if (enc === '') return 'utf8';
...
function convertToValidSignal(signal) {
  ...
  if (typeof signal === 'string') {
    const signalName = signals[toUpperCase(signal)];
