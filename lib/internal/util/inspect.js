...
const escapeFn = (str) => meta[charCodeAt(str, 0)];
...
function strEscape(str) {
  ...
  // double quotes.
  if (stringIncludes(str, "'")) {
    // This invalidates the charCode and therefore can not be matched for
    // anymore.
    if (!stringIncludes(str, '"')) {
      singleQuote = -1;
    } else if (!stringIncludes(str, '`') && !stringIncludes(str, '${')) {
      singleQuote = -2;
    }
    if (singleQuote !== 39) {
  ...
  if (str.length > 100) {
    str = replace(str, escapeReplace, escapeFn);
    return addQuotes(str, singleQuote);
  }
...
function formatRaw(ctx, value, recurseTimes, typedArray) {
  ...
  if (recurseTimes > ctx.depth && ctx.depth !== null) {
    let constructorName =
      stringSlice(getCtxStyle(value, constructor, tag), 0, -1);
    if (constructor !== null)
...
function getBoxedBase(value, ctx, keys, constructor, tag) {
  ...
  if (keys.length !== 0 || ctx.stylize === stylizeNoColor)
    return base;
  return ctx.stylize(base, toLowerCase(type));
}
...
function formatError(err, constructor, tag, ctx) {
  ...
  if (constructor === null ||
      endsWith(name, 'Error') &&
      startsWith(stack, name) &&
      (stack.length === len || stack[len] === ':' || stack[len] === '\n')) {
    let fallback = 'Error';
    if (constructor === null) {
      const start =
        match(stack, /^([A-Z][a-z_ A-Z0-9[\]()-]+)(?::|\n {4}at)/) ||
        match(stack, /^([a-z_A-Z0-9-]*Error)$/);
      fallback = start && start[1] || '';
      len = fallback.length;
      fallback = fallback || 'Error';
    }
    const prefix = stringSlice(getPrefix(constructor, tag, fallback), 0, -1);
    if (name !== prefix) {
      if (stringIncludes(prefix, name)) {
        if (len === 0) {
          stack = `${prefix}: ${stack}`;
        } else {
          stack = `${prefix}${stringSlice(stack, len)}`;
        }
      } else {
        stack = `${prefix} [${name}]${stringSlice(stack, len)}`;
      }
    }
  }
  // Ignore the error message if it's contained in the stack.
  let pos = err.message && stringIndexOf(stack, err.message) || -1;
  if (pos !== -1)
    pos += err.message.length;
  // Wrap the error in brackets in case it has no stack trace.
  const stackStart = stringIndexOf(stack, '\n    at', pos);
  if (stackStart === -1) {
    stack = `[${stack}]`;
  } else if (ctx.colors) {
    // Highlight userland code and node modules.
    let newStack = stringSlice(stack, 0, stackStart);
    const lines = split(stringSlice(stack, stackStart + 1), '\n');
    for (const line of lines) {
      const core = match(line, coreModuleRegExp);
      if (core !== null && NativeModule.exists(core[1])) {
        ...
        while (nodeModule = nodeModulesRegExp.exec(line)) {
          ...
          newStack += stringSlice(line, pos, nodeModule.index + 14);
          newStack += ctx.stylize(nodeModule[1], 'module');
          pos = nodeModule.index + nodeModule[0].length;
        }
        newStack += pos === 0 ? line : stringSlice(line, pos);
      }
    }
    stack = newStack;
  }
...
function formatNamespaceObject(ctx, value, recurseTimes, keys) {
  ...
  for (var i = 0; i < keys.length; i++) {
    try {
      ...
      const pos = stringLastIndexOf(output[i], ' ');
      // We have to find the last whitespace and have to replace that value as
      // it will be visualized as a regular string.
      output[i] = stringSlice(output[i], 0, pos + 1) +
...
function formatArrayBuffer(ctx, value) {
  ...
  let str = replace(slice, /(.{2})/g, '$1 ').trim();
...
function formatIterator(ctx, value, recurseTimes, keys, braces) {
  ...
  if (isKeyValue) {
    ...
    braces[0] = replace(braces[0], / Iterator] {$/, ' Entries] {');
...
function formatProperty(ctx, value, recurseTimes, key, type) {
  ...
  if (typeof key === 'symbol') {
    const tmp = replace(key.toString(), strEscapeSequencesReplacer, escapeFn);
    name = `[${ctx.stylize(tmp, 'symbol')}]`;
  } else if (desc.enumerable === false) {
    name = `[${replace(key, strEscapeSequencesReplacer, escapeFn)}]`;
...
function isBelowBreakLength(ctx, output, start, base) {
  ...
  return base === '' || !stringIncludes(base, '\n');
}
...
const firstErrorLine = (error) => split(error.message, '\n')[0];
