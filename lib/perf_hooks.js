...
const kObserverType = Symbol('observer type');

const observers = {};
const observableTypes = new SafeSet([
  'node',
  'mark',
  'measure',
  'gc',
...
class PerformanceObserver extends AsyncResource {
  ...
  observe(options = {}) {
    ...
    const hasEntryTypes = typeof entryTypes !== 'undefined';
    const hasType = typeof type !== 'undefined';

    if (!hasEntryTypes && !hasType) {
      throw new TypeError('The entryTypes or type option must be passed');
    }
    if (hasEntryTypes && hasType) {
      throw new TypeError('The entryTypes and type options are mutually exclusive');
    }

    if (this[kObserverType] === 'undefined') {
      if (hasEntryTypes) {
        this[kObserverType] = 'multiple';
      } else {
        this[kObserverType] = 'single';
      }
    } else if (this[kObserverType] === 'single' && hasEntryTypes) {
      throw new InvalidModificationError();
    } else if (this[kObserverType] === 'multiple' && hasType) {
      throw new InvalidModificationError();
    }
    
    const observerCountsGC = observerCounts[NODE_PERFORMANCE_ENTRY_TYPE_GC];

    if (this[kObserverType] === 'multiple') {
